...
options
{
	language=CSharp2;
	output=AST;
	ASTLabelType=IIntellisenseNode;
}
...
@parser::namespace { HqlIntellisense }
@lexer::namespace { HqlIntellisense }
...
fromClause
	: FROM^ { WeakKeywords(); } fromRange ( fromJoin | COMMA! { WeakKeywords(); } fromRange )*
	  |FROM^ { ProduceFromCompletion((IIntellisenseNode)root_0); }
	;
...
@after {
   if (c != null)
   {
      IIntellisenseNode mc = (IIntellisenseNode) adaptor.Create(METHOD_CALL, "||");
      IIntellisenseNode concat = (IIntellisenseNode) adaptor.Create(IDENT, "concat");
      mc.AddChild(concat);
      mc.AddChild((IIntellisenseNode) retval.Tree);
      retval.Tree = mc;
   }
}
...
atom
	 : p=primaryExpression
		(
			DOT^ {ProduceAtomCompletion($p.tree);}
			|DOT^ identifier
				( options { greedy=true; } :
...
exprList
@after {
   IIntellisenseNode root = (IIntellisenseNode) adaptor.Create(EXPR_LIST, "exprList");
   root.AddChild((IIntellisenseNode)retval.Tree);
   retval.Tree = root;
}
